...
영업사원이 기기 조작에 시간을 쓰기보다 주문과 고객 응대 자체에 집중할 수 있도록, 반복적인 조작과 입력 과정을 줄이는 방향으로 기능을 개선했습니다.
...
1인 Android 개발자로 기획부터 개발, 리팩터링, 운영 대응까지 전 과정을 담당했습니다.

- 주문등록, 수금관리, 반품등록, 전표조회, 구매요청 등 주요 업무 기능 개발
- KDC 블루투스 스캐너 및 TSC 프린터 연동
- SQLite 기반 검색 이력 및 작성 중 목록 임시 저장 기능 구현
- JWT 인증 및 401 전역 처리 적용
- 루팅 탐지 및 앱 위변조 검증 등 보안 대응
- Activity 중심 구조를 MVVM + Repository 구조로 점진적 리팩터링

> 서버 API 명세는 별도 백엔드 담당자와 협의했으며, 서버/백엔드 구현과 디자인 리소스 제작은 본 문서에서 다루지 않습니다.

---

## 문제 해결 사례

운영 과정에서 특히 기억에 남았던 문제 해결 사례를 정리했습니다.

### 사례 1. 기능이 늘어나며 복잡해진 구조를 점진적으로 정리한 경험
...
| 구분 | 내용 |
|---|---|
| 기술적 성과 | 139개 파일에 걸쳐 구조를 점진적으로 전환하며 ViewModel·Repository 기반 책임 분리를 적용 |
...
### 사례 3. 토큰 만료 상황에서 사용자가 길을 잃지 않도록 한 경험
...
현장에서 반복적으로 사용되다 보니 자연스럽게 추가하게 된 작은 개선들입니다.

| 개선 | 내용 |
|---|---|
| **블루투스 자동 재연결** | 매장 방문마다 스캐너·프린터를 다시 페어링해야 하는 불편을 줄이기 위해, 마지막 연결 기기 주소를 SharedPreferences에 저장하고 앱 실행 시 자동으로 재연결 |
...
| 화면 | 구현 포인트 |
|---|---|
| 로그인 | 토큰 발급 및 예외 상황 대응 |
| 주문/수금 | 현장 입력 흐름에 맞춘 자동완성·임시 저장 |
| 인쇄 옵션 | 현장에서 바로 출력할 수 있는 프린터 연동 |
...
**이번 경험을 통해 얻은 교훈**
처음에는 현장에서 필요한 기능을 빠르게 만드는 것이 가장 중요하다고 생각했습니다. 하지만 기능이 늘어나고 운영 이슈가 쌓일수록, 처음의 작은 선택들이 유지보수 비용으로 돌아온다는 것을 직접 경험했습니다. 이후에는 개발 속도뿐 아니라, 운영하면서 어떤 변화가 생길지까지 함께 고려하며 구조를 설계하려고 노력하고 있습니다.
