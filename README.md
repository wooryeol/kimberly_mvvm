...
3. 거래처를 검색해 선택한다.
4. 제품을 바코드로 스캔하거나, 검색창에 두 글자만 입력하면 **이전에 주문했던 제품 목록이 자동완성**으로 뜬다.
5. 여러 제품을 담는 도중 전화가 와서 앱이 백그라운드로 내려가거나 통신이 끊겨도, 다시 들어오면 **"이전에 작성하던 목록을 이어서 쓸지" 팝업**이 뜬다.
...
**해결**
주문·반품 화면에 진입할 때마다 작성 중인 품목 목록을 SQLite에 저장하고, 앱 재진입 시 저장된 데이터가 있으면 "이전 작업 이어서 진행" 또는 "기존 데이터 삭제 후 새로 작성" 중 하나를 선택할 수 있는 팝업을 제공했다.

**Outcome**

| 구분 | 내용 |
|---|---|
| 기술적 성과 | `db/DBHelper.kt` 기반으로 화면별 임시 목록을 저장·복구하는 공통 패턴 구현 |
| 엔지니어링 효과 | 화면 전환·앱 종료로 인한 데이터 손실 위험을 구조적으로 제거 |
...
| 개선 | 내용 |
|---|---|
| **검색 이력 기반 상품 자동완성** | 동일 상품을 반복 검색해야 하는 비효율을 줄이기 위해, SQLite에 검색 이력을 저장하고 두 글자 이상 입력 시 자동완성 목록을 제공 |
